...
class FlagsField(BitField):
    ...
    def i2repr(self, pkt, x):
        ...
        return r
    def i2h(self, pkt, x):
        class FlagsWrapper(int):
            def __eq__(mself, rhs):
                # Fixme: check that this use of 'None' is not a problem
                return int(mself) == self.any2i(None, rhs)
            def __add__(mself, rhs):
                return int(mself) + self.any2i(None, rhs)
            def __sub__(mself, rhs):
                return int(mself) - self.any2i(None, rhs)
            def __and__(mself, rhs):
                return int(mself) & self.any2i(None, rhs)
            def __xor__(mself, rhs):
                return int(mself) ^ self.any2i(None, rhs)
            def __or__(mself, rhs):
                return int(mself) | self.any2i(None, rhs)           
        return FlagsWrapper(x)
